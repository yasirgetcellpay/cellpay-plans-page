...
const ATT = () => {
  ...
  const handleAmountChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (num <= 300) setAmount(val);
  }, []);

  const phoneDigits = phone.replace(/\D/g, "");
  const amountNum = amount ? parseInt(amount, 10) : 0;
  const isValid = phoneDigits.length === 10 && amountNum >= 5 && amountNum <= 300 && confirmed && agreedTerms;
  ...
  return (
    <div className="min-h-screen bg-background font-sans antialiased">
      ...
      <section className="bg-[hsl(196,100%,44%)] text-primary-foreground">
        ...
        <div className="bg-card rounded-xl shadow-lg border border-border p-4 sm:p-6 text-center">
          ...
          <div className="relative mb-3">
            ...
            <input type="tel" value={phone} onChange={handlePhoneChange} placeholder="(XXX) XXX-XXXX"
              className="w-full h-10 sm:h-12 pl-10 sm:pl-11 pr-4 rounded-lg border border-input bg-background text-sm sm:text-base text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-[hsl(196,100%,44%)] focus:border-transparent text-center" />
          </div>
          ...
          <div className="relative mb-1">
            ...
            <input type="text" inputMode="numeric" value={amount} onChange={handleAmountChange} placeholder="$5 - $300"
              className="w-full h-10 sm:h-12 pl-10 sm:pl-11 pr-4 rounded-lg border border-input bg-background text-sm sm:text-base text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-[hsl(196,100%,44%)] focus:border-transparent text-center" />
          </div>
          ...
          {quickAmounts.map((amt) => {
            ...
            return (
              <button key={amt} type="button" onClick={() => setAmount(String(amt))}
                className={`rounded-lg border-2 py-2 text-xs sm:text-sm font-bold transition-all active:scale-[0.96] ${isSelected ? "border-[hsl(196,100%,44%)] bg-[hsl(196,100%,44%)] text-primary-foreground" : "border-border bg-muted text-foreground hover:border-[hsl(196,100%,44%)]"}`}>
                ${amt}
              </button>
            );
          })}
      ...
      <div className="max-w-[420px] mx-auto px-4 pb-8 sm:pb-12">
        ...
        <label className="flex items-start gap-2 mb-3 cursor-pointer">
          <input type="checkbox" checked={confirmed} onChange={(e) => setConfirmed(e.target.checked)} className="mt-0.5 h-4 w-4 rounded border-input accent-[hsl(196,100%,44%)]" />
        ...
        <label className="flex items-start gap-2 mb-6 cursor-pointer">
          <input type="checkbox" checked={agreedTerms} onChange={(e) => setAgreedTerms(e.target.checked)} className="mt-0.5 h-4 w-4 rounded border-input accent-[hsl(196,100%,44%)]" />
          <span className="text-[11px] sm:text-xs text-muted-foreground leading-relaxed">Agree with AT&amp;T Product Policies and Sales.{" "}<a href="https://www.att.com/legal/terms.attWebsiteTermsOfUse.html" className="text-[hsl(196,100%,44%)] underline font-semibold">View More</a></span>
        </label>
        <div className="flex justify-center">
          <button type="button" disabled={!isValid} className="h-[44px] sm:h-[48px] px-10 sm:px-14 rounded-lg bg-[hsl(196,100%,44%)] hover:bg-[hsl(196,100%,38%)] disabled:opacity-50 disabled:cursor-not-allowed text-primary-foreground font-bold text-base sm:text-lg transition-colors active:scale-[0.97]">PAY NOW</button>
        </div>
      ...
      <footer className="bg-cellpay-dark text-muted-foreground py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="border-t border-muted pt-6 text-center">
            ...
            <p className="text-[10px] leading-relaxed max-w-4xl mx-auto opacity-50 mt-3">AT&amp;T® is a registered trademark of AT&amp;T Intellectual Property. All carrier names and trademarks are property of their respective owners.</p>
          </div>
        </div>
      </footer>
      <div className="bg-[hsl(196,100%,44%)] text-primary-foreground py-3 text-[10px] md:text-xs">
