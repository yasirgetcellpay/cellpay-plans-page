...
const PagePlus = () => {
  useEffect(() => { applySeoHead({ title: 'Page Plus Cellular Refill Online | CellPay', description: 'Top up Page Plus Cellular prepaid plans with CellPay. Secure online refills delivered instantly to your Page Plus number.' }); }, []);
  ...
  const handlePlanSelect = (plan: { price: string }) => {
    ...
    navigate("/checkout", { state: { phone, amount: plan.price.replace("$", ""), carrierSlug: "pageplus", carrierName: "PagePlus Cellular", brandColor } });
  };
      ...
      <nav className="sticky top-0 z-50 bg-card border-b-4 shadow-sm" style={{ borderColor: brandColor }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="relative flex justify-center h-14 sm:h-20 items-center">
            <BackButton />
            <img src={pageplusLogo} alt="Page Plus" className="h-[32px] sm:h-[44px] w-auto object-contain" />
          </div>
        </div>
      </nav>
