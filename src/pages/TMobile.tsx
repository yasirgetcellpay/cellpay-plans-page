...
const BRAND_HOVER = "hsl(330, 100%, 38%)";
...
const TMobile = () => {
  ...
  const handleAmountChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (num <= 500) setAmount(val);
  }, []);

  const phoneDigits = phone.replace(/\D/g, "");
  const amountNum = amount ? parseInt(amount, 10) : 0;
  const isValid = phoneDigits.length === 10 && amountNum >= 10 && amountNum <= 500 && confirmed && agreedTerms;
        ...
        <div className="bg-card rounded-xl shadow-lg border border-border p-5 sm:p-8">
          ...
          <div className="relative mb-5 sm:mb-6">
            ...
            <input
              ...
              className="w-full h-10 sm:h-12 pl-10 sm:pl-11 pr-4 rounded-lg border border-input bg-background text-sm sm:text-base text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-[hsl(330,100%,45%)] focus:border-transparent"
            />
          </div>
          ...
          <div className="relative mb-4 sm:mb-5">
            ...
            <input
              ...
              placeholder="Enter an amount between 10 - 500"
              className="w-full h-10 sm:h-12 pl-10 sm:pl-11 pr-4 rounded-lg border border-input bg-background text-sm sm:text-base text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-[hsl(330,100%,45%)] focus:border-transparent"
            />
          </div>
              ...
              return (
                <button
                  ...
                  className={`rounded-lg border-2 py-2 text-xs sm:text-sm font-bold transition-all active:scale-[0.96] ${
                    isSelected
                      ? "border-[hsl(330,100%,45%)] bg-[hsl(330,100%,45%)] text-primary-foreground"
                      : "border-border bg-muted text-foreground hover:border-[hsl(330,100%,45%)]"
                  }`}
                >
                  ${amt}
                </button>
          ...
          <label className="flex items-start gap-2 mb-3 cursor-pointer">
            <input
              ...
              className="mt-0.5 h-4 w-4 rounded border-input accent-[hsl(330,100%,45%)]"
            />
          ...
          <label className="flex items-start gap-2 mb-6 cursor-pointer">
            ...
            />
            <span className="text-[11px] sm:text-xs text-muted-foreground leading-relaxed">
              Agree with T-Mobile Product Policies and Sales.{" "}
              <a href="https://www.t-mobile.com/terms-conditions" className="text-[hsl(330,100%,45%)] underline font-semibold">
                View More
              </a>
            </span>
          </label>
          ...
          <div className="flex justify-center">
            <button
              type="button"
              disabled={!isValid}
              className="h-[48px] sm:h-[52px] px-12 sm:px-16 rounded-lg bg-[hsl(330,100%,45%)] hover:bg-[hsl(330,100%,38%)] disabled:opacity-50 disabled:cursor-not-allowed text-primary-foreground font-bold text-base sm:text-lg transition-colors active:scale-[0.97]"
            >
              PAY NOW
            </button>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="border-t border-muted pt-6 text-center">
            ...
            <p className="text-[10px] leading-relaxed max-w-4xl mx-auto opacity-50 mt-3">
              T-Mobile® is a registered trademark of T-Mobile USA, Inc. All carrier names and trademarks are property of their respective owners.
            </p>
          </div>
        </div>
      </footer>
