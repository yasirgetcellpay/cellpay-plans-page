    ...
    <div className="min-h-screen bg-background font-sans antialiased">
          ...
          {plans.map((plan, i) => {
            ...
            return (
              <button key={i} type="button" onClick={() => setSelectedPlan(i)}
                className={`rounded-lg border-2 text-left overflow-hidden transition-all active:scale-[0.97] ${isSelected ? "border-[hsl(195,85%,50%)]" : "border-border hover:border-[hsl(195,85%,50%)]/50"}`}>
      ...
      <div className="max-w-[420px] mx-auto px-4 pb-8 sm:pb-12">
        ...
        <label className="flex items-start gap-2 mb-3 cursor-pointer">
          <input type="checkbox" checked={confirmed} onChange={(e) => setConfirmed(e.target.checked)} className="mt-0.5 h-4 w-4 rounded border-input accent-[hsl(195,85%,50%)]" />
        ...
        <label className="flex items-start gap-2 mb-6 cursor-pointer">
          <input type="checkbox" checked={agreedTerms} onChange={(e) => setAgreedTerms(e.target.checked)} className="mt-0.5 h-4 w-4 rounded border-input accent-[hsl(195,85%,50%)]" />
          <span className="text-[11px] sm:text-xs text-muted-foreground leading-relaxed">Agree with H2O Wireless Product Policies and Sales.{" "}<a href="https://www.h2owireless.com/terms-and-conditions" className="text-[hsl(195,85%,50%)] underline font-semibold">View More</a></span>
        </label>
        <div className="flex justify-center">
          <button type="button" disabled={!isValid} className="h-[44px] sm:h-[48px] px-10 sm:px-14 rounded-lg bg-[hsl(195,85%,50%)] hover:bg-[hsl(195,85%,42%)] disabled:opacity-50 disabled:cursor-not-allowed text-primary-foreground font-bold text-base sm:text-lg transition-colors active:scale-[0.97]">PAY NOW</button>
        </div>
