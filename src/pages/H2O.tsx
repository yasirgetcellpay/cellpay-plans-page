...
const H2O = () => {
  ...
  const handlePlanSelect = (plan: { price: string }) => {
    ...
    navigate("/checkout", { state: { phone, amount: plan.price.replace("$", ""), carrierSlug: "h2o", carrierName: "H2O Wireless", brandColor } });
  };
      ...
      <nav className="sticky top-0 z-50 bg-card border-b-4 shadow-sm" style={{ borderColor: brandColor }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="relative flex justify-center h-14 sm:h-20 items-center">
            <BackButton />
            <img src={h2oLogo} alt="H2O Wireless" className="h-[32px] sm:h-[44px] w-auto object-contain" />
          </div>
        </div>
      </nav>
