...
const UltraMobile = () => {
  ...
  return (
    <div className="min-h-screen bg-background font-sans antialiased">
      <nav className="sticky top-0 z-50 bg-[hsl(270,50%,40%)] border-b-4 border-[hsl(270,50%,30%)] shadow-sm">
      ...
      <section className="bg-[hsl(270,50%,40%)] text-primary-foreground">
          ...
          {plans.map((price, i) => {
            ...
            return (
              <button key={i} type="button" onClick={() => setSelectedPlan(i)}
                className={`rounded-lg border-2 py-2 text-xs sm:text-sm font-bold transition-all active:scale-[0.96] ${isSelected ? "border-[hsl(270,50%,40%)] bg-[hsl(270,50%,40%)] text-primary-foreground" : "border-border bg-muted text-foreground hover:border-[hsl(270,50%,40%)]"}`}>
                {price}
              </button>
            );
          })}
      ...
      <div className="max-w-[420px] mx-auto px-4 pb-8 sm:pb-12">
        ...
        <label className="flex items-start gap-2 mb-3 cursor-pointer">
          <input type="checkbox" checked={confirmed} onChange={(e) => setConfirmed(e.target.checked)} className="mt-0.5 h-4 w-4 rounded border-input accent-[hsl(270,50%,40%)]" />
        ...
        <label className="flex items-start gap-2 mb-6 cursor-pointer">
          <input type="checkbox" checked={agreedTerms} onChange={(e) => setAgreedTerms(e.target.checked)} className="mt-0.5 h-4 w-4 rounded border-input accent-[hsl(270,50%,40%)]" />
          <span className="text-[11px] sm:text-xs text-muted-foreground leading-relaxed">Agree with Ultra Mobile Product Policies and Sales.{" "}<a href="https://www.ultramobile.com/terms-conditions/" className="text-[hsl(270,50%,40%)] underline font-semibold">View More</a></span>
        </label>
        <div className="flex justify-center">
          <button type="button" disabled={!isValid} className="h-[44px] sm:h-[48px] px-10 sm:px-14 rounded-lg bg-[hsl(270,50%,40%)] hover:bg-[hsl(270,50%,32%)] disabled:opacity-50 disabled:cursor-not-allowed text-primary-foreground font-bold text-base sm:text-lg transition-colors active:scale-[0.97]">PAY NOW</button>
        </div>
      ...
      <div className="bg-[hsl(270,50%,40%)] text-primary-foreground py-3 text-[10px] md:text-xs">
